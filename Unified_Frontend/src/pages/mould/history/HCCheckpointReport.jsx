...
export default function HCCheckPointReport() {
  ...
  // 2. When Mould is chosen, fetch its HC instances from history
  useEffect(() => {
  ...
  // 3. Fetch Header & Checkpoint details
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        const hRes = await axios.get(
          `${BASE}/MouldMaintenanceHistoryHC/HCCheckpointDetails/HCHeaderDetails`,
          {
            params: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "HC_Report");
    XLSX.writeFile(workbook, `HC_Report_${activeMouldName || "Mould"}_Inst_${activeInstance}.xlsx`);
  };
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text("Health Check (HC) Checkpoint Report", 14, 15);
    doc.setFontSize(9);
    ...
    doc.save(`HC_Report_${activeMouldName}_Inst_${activeInstance}.pdf`);
  };
      ...
      <div className="space-y-6 pb-8" ref={printRef}>
        ...
        <div className="bg-white  border border-slate-200  rounded-xl p-5 shadow-2xs">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold tracking-tight text-slate-800  flex items-center gap-2">
                <MdHealing className="text-cyan-400" size={24} />
                <span>Health Check (HC) Checkpoint Report</span>
              </h1>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    `/HCCheckpointImages?mouldName=${encodeURIComponent(activeMouldName)}&instance=${activeInstance}`
                  )
                }
          ...
          <div className="mt-5 pt-4 border-t border-slate-200  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-center">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700  mb-1.5 block flex items-center gap-1.5">
                <MdTune className="text-amber-400" size={16} />
                  ...
                  <tr>
                    <td colSpan="10" className="py-8 text-center text-slate-400 text-xs">
                      {activeMouldID
                        ...
                        : "Please select a Mould and HC Instance above to load inspection report."}
                    </td>
                  </tr>
                ) : (
