...
export default function HCCheckpointImages() {
  ...
  useEffect(() => {
    ...
    const fetchInstances = async () => {
      ...
      try {
        const res = await axios.get(`${BASE}/MouldMaintenanceHistoryHC/HcHistoryDetailTable`, {
        ...
        console.error("Error fetching HC instances:", err);
        setInstanceList([]);
      } finally {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE}/MouldMaintenanceHistoryHC/HCCheckpointDetails/get-checkpoint-images`, {
      ...
      <div className="space-y-6 pb-8">
        ...
        <div className="bg-white  border border-slate-200  rounded-xl p-5 shadow-2xs">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold tracking-tight text-slate-800  flex items-center gap-2">
                <MdPhotoLibrary className="text-amber-400" size={24} />
                <span>HC Checkpoint Inspection Images</span>
              </h1>
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                onClick={() =>
                  navigate(
                    `/HCCheckPointReport?mouldName=${encodeURIComponent(activeMouldName)}&instance=${activeInstance}&mouldID=${activeMouldID}`
                  )
                }
          ...
          <div className="mt-5 pt-4 border-t border-slate-200  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-center">
            ...
            {/* HC Instance Selector */}
            <div>
              <label className="text-xs font-semibold text-slate-700  mb-1.5 block flex items-center gap-1.5">
                <MdTune className="text-cyan-400" size={16} />
                <span>Select HC Instance</span>
              </label>
              <Select
              ...
              {images.map((img, index) => {
                ...
                return (
                  <div
                    ...
                    className="group bg-slate-50  border border-slate-200  rounded-xl p-2.5 cursor-pointer transition-all hover:border-amber-500 hover:shadow-lg flex flex-col items-center"
                  >
            ...
            <div className="flex flex-col items-center justify-center h-48 text-slate-400 text-xs gap-1">
              ...
              <span className="text-[11px] text-slate-500">
                {activeMouldName
                  ...
                  : "Please select a Mould and HC Instance from the searcher above to inspect images."}
              </span>
            </div>
          )}
        </div>
            ...
            >
              ...
              <div className="w-full pt-3 mt-3 border-t border-[#262a34] flex justify-end">
                <a
                  ...
                  download={`HC_Checkpoint_${preview.checkpoint}_${activeMouldName}.jpg`}
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-amber-600 hover:bg-amber-500 text-white"
                >
