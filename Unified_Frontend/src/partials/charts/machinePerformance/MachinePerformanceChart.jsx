...
const MachinePerformanceChart = () => {
  ...
  const value = machineOEE?.[0]?.PerformancePercent || 0;
    ...
    datasets: [
      {
        ...
        backgroundColor: [performanceColor, "rgba(183,188,213,1)"], // blue
        borderWidth: 0,
      },
    ],
  }), [value]);
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        enabled: true,
        callbacks: {
          label: (ctx) => ctx.dataIndex === 0 ? `Performance: ${ctx.raw}%` : null,
        },
      },
    },
